...
from clavi_agent.tools.bash_tool import BackgroundShellManager, BashKillTool, BashOutputTool, BashTool
...
@pytest.mark.asyncio
async def test_foreground_command_uses_workspace_dir(tmp_path):
    """BashTool should execute commands from its configured workspace."""
    ...
    assert result.success
    assert Path(result.stdout.strip()).resolve() == workspace_dir.resolve()
