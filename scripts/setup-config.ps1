# Clavi Agent Configuration Setup Script for Windows
...
$CONFIG_DIR = Join-Path $env:USERPROFILE ".clavi-agent\config"

Write-ColorOutput "╔════════════════════════════════════════════════╗" -Color "Cyan"
Write-ColorOutput "║   Clavi Agent Configuration Setup              ║" -Color "Cyan"
Write-ColorOutput "╚════════════════════════════════════════════════╝" -Color "Cyan"
Write-Host ""

# Step 1: Create config directory
Write-ColorOutput "[1/3] Creating configuration directory..." -Color "Blue"
if (Test-Path $CONFIG_DIR) {
    # Auto backup existing config
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $BACKUP_DIR = Join-Path $env:USERPROFILE ".clavi-agent\config.backup.$timestamp"
...
Write-ColorOutput "[2/3] Downloading configuration files..." -Color "Blue"
...
Write-ColorOutput "╚════════════════════════════════════════════════╝" -Color "Green"
Write-Host ""
Write-ColorOutput "[3/3] Installing runtime dependencies..." -Color "Blue"
$setupRuntime = Get-Command "clavi-agent-setup-runtime" -ErrorAction SilentlyContinue
if (-not $setupRuntime) {
    Write-ColorOutput "   clavi-agent-setup-runtime command not found" -Color "Red"
    Write-ColorOutput "   Please ensure Clavi Agent is installed before running this setup script" -Color "Yellow"
    exit 1
}

& $setupRuntime.Source --config (Join-Path $CONFIG_DIR "config.yaml")
if ($LASTEXITCODE -ne 0) {
    Write-ColorOutput "   Runtime dependency setup failed" -Color "Red"
    exit $LASTEXITCODE
}
Write-ColorOutput "   Runtime dependencies ready" -Color "Green"
Write-Host ""
...
Write-Host ""
Write-ColorOutput "1. Install Clavi Agent:" -Color "Yellow"
Write-ColorOutput "   pipx install git+https://github.com/MiniMax-AI/clavi-agent.git" -Color "Green"
Write-Host ""
...
Write-Host ""
Write-ColorOutput "3. Start using Clavi Agent:" -Color "Yellow"
Write-ColorOutput "   clavi-agent                              # Use current directory" -Color "Green"
Write-ColorOutput "   clavi-agent --workspace C:\path\to\project # Specify workspace" -Color "Green"
Write-ColorOutput "   clavi-agent --help                      # Show help" -Color "Green"
Write-Host ""
