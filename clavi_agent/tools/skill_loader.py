...
class Skill:
    ...
    @property
    def skill_root(self) -> Optional[Path]:
        """Return the root directory of the skill package."""
        return self.skill_path.parent if self.skill_path else None
    ...
    def to_prompt(self) -> str:
        ...
        if self.skill_root:
            location_block = (
                ...
                "Use this directory as the base path for this skill's scripts and resources.\n\n"
            )
        return f"""
# Skill: {self.name}
...
class SkillLoader:
    ...
    def get_skills_metadata(self) -> List[Dict[str, str]]:
        """Return loaded skill metadata for storage or prompt composition."""
        return [
            {
                "name": skill.name,
...
def build_skills_description_prompt(skills: List[Dict[str, Any]] | None) -> str:
    """Build a compact prompt section from persisted skill metadata."""
    if not skills:
        return ""

    lines = [
        "## Available Skills",
        "This agent has the following installed skills. Use `get_skill` to load the full guidance for a skill when needed.",
    ]
