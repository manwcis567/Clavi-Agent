...
from clavi_agent.tools.mcp_loader import load_mcp_tools_async
from clavi_agent.tools.note_tool import RecallNoteTool, SessionNoteTool


async def demo_full_agent():
    """Demo: Full-featured agent with all capabilities."""
    print("\n" + "=" * 60)
    print("Full Clavi Agent - All Features Enabled")
    print("=" * 60)
    ...
    if not config_path.exists():
        ...
        print("   cp clavi_agent/config/config-example.yaml clavi_agent/config/config.yaml")
        return
...
"""
        ...
        try:
            mcp_tools = await load_mcp_tools_async(config_path="clavi_agent/config/mcp.json")
            if mcp_tools:
